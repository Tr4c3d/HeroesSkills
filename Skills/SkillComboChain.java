...
public class SkillComboChain extends PassiveSkill implements Listener{
	
	public HeroesSkillTree hst = (HeroesSkillTree)Bukkit.getServer().getPluginManager().getPlugin("HeroesSkillTree");

    public SkillComboChain(Heroes plugin) {
        super(plugin, "ComboChain");
        setDescription("Passive $1 chance of having $2 after DoubleCombo to deal a third stronger attack");
        setUsage("/skill ComboChain");
        setArgumentRange(0, 0);
        setIdentifiers(new String[]{"skill ComboChain"});
    ...
    public void onWeaponDamage(WeaponDamageEvent event){
    	if(event.getDamager() instanceof Hero){
    		...
    		if(hero.hasEffect("ComboC")){
    			int multiplier = (int) (SkillConfigManager.getUseSetting(hero, this, "multiplier", 1.0, false) +
                        (SkillConfigManager.getUseSetting(hero, this, "multiplier-increase", 0.0, false) * hero.getSkillLevel(this)));
                if(hst != null) multiplier += (SkillConfigManager.getUseSetting(hero, this, "hst-multiplier", 0.0, false) * (hst.getSkillLevel(hero, this) - 1));
                multiplier = multiplier > 0 ? multiplier : 0;
                event.setDamage((int) (event.getDamage() * multiplier));
                Messaging.broadcast(plugin, hero.getName() + " combo chains!");
                if(hero.hasEffect("ComboFinal")){
                	int duration = (int) (SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("ComboFinal"), Setting.DURATION, 1000, false) +
	    	        		(SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("ComboFinal"), Setting.DURATION_INCREASE, 0, false) * hero.getSkillLevel(plugin.getSkillManager().getSkill("ComboFinal"))));
	    	        if(hst != null) duration += (SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("ComboFinal"), "hst-duration", 0, false) * (hst.getSkillLevel(hero, plugin.getSkillManager().getSkill("ComboFinal")) - 1));
	    	        duration = duration > 0 ? duration : 0;
                	hero.addEffect(new ExpirableEffect(plugin.getSkillManager().getSkill("ComboFinal"), "CFinal", duration));
                }
                hero.removeEffect(hero.getEffect("ComboC"));
    		}
    		if(hero.hasEffect("ComboChain") && hero.hasEffect("CChain") && !hero.hasEffect("ComboC")){
    			double chance = (SkillConfigManager.getUseSetting(hero, this, Setting.CHANCE.node(), 0.2, false) +
    	        ...
    	        if(Math.random() <= chance){
	    	        ...
	    	        if(hst != null) duration += (SkillConfigManager.getUseSetting(hero, this, "hst-duration", 0, false) * (hst.getSkillLevel(hero, this) - 1));
	    	        duration = duration > 0 ? duration : 0;
	    			hero.addEffect(new ExpirableEffect(this, "ComboC", duration));
	    			hero.removeEffect(hero.getEffect("CChain"));
    	        }
    		}
    	}
    }
