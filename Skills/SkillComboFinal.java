...
public class SkillComboFinal extends PassiveSkill implements Listener{
	
	public HeroesSkillTree hst = (HeroesSkillTree)Bukkit.getServer().getPluginManager().getPlugin("HeroesSkillTree");

    public SkillComboFinal(Heroes plugin) {
        super(plugin, "ComboFinal");
        setDescription("Passive $1 chance of having $2 after ComboFinal to deal a final stronger attack");
        setUsage("/skill ComboFinal");
        setArgumentRange(0, 0);
        setIdentifiers(new String[]{"skill ComboFinal"});
    ...
    public void onWeaponDamage(WeaponDamageEvent event){
    	if(event.getDamager() instanceof Hero){
    		...
    		if(hero.hasEffect("ComboF")){
    			int multiplier = (int) (SkillConfigManager.getUseSetting(hero, this, "multiplier", 1.0, false) +
                        (SkillConfigManager.getUseSetting(hero, this, "multiplier-increase", 0.0, false) * hero.getSkillLevel(this)));
                if(hst != null) multiplier += (SkillConfigManager.getUseSetting(hero, this, "hst-multiplier", 0.0, false) * (hst.getSkillLevel(hero, this) - 1));
                multiplier = multiplier > 0 ? multiplier : 0;
                event.setDamage((int) (event.getDamage() * multiplier));
                Messaging.broadcast(plugin, hero.getName() + " combo finals!");
                if(hero.hasEffect("AsuraStrike")){
                	int duration = (int) (SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("AsuraStrike"), Setting.DURATION, 1000, false) +
	    	        		(SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("AsuraStrike"), Setting.DURATION_INCREASE, 0, false) * hero.getSkillLevel(plugin.getSkillManager().getSkill("AsuraStrike"))));
	    	        if(hst != null) duration += (SkillConfigManager.getUseSetting(hero, plugin.getSkillManager().getSkill("AsuraStrike"), "hst-duration", 0, false) * (hst.getSkillLevel(hero, plugin.getSkillManager().getSkill("AsuraStrike")) - 1));
	    	        duration = duration > 0 ? duration : 0;
                	hero.addEffect(new ExpirableEffect(plugin.getSkillManager().getSkill("AsuraStrike"), "Asura", duration));
                }
                hero.removeEffect(hero.getEffect("ComboF"));
    		}
    		if(hero.hasEffect("ComboFinal") && hero.hasEffect("CFinal") && !hero.hasEffect("FinalC")){
    			double chance = (SkillConfigManager.getUseSetting(hero, this, Setting.CHANCE.node(), 0.2, false) +
    	        ...
    	        if(Math.random() <= chance){
	    	        ...
	    	        if(hst != null) duration += (SkillConfigManager.getUseSetting(hero, this, "hst-duration", 0, false) * (hst.getSkillLevel(hero, this) - 1));
	    	        duration = duration > 0 ? duration : 0;
	    			hero.addEffect(new ExpirableEffect(this, "ComboF", duration));
	    			hero.removeEffect(hero.getEffect("CFinal"));
    	        }
    		}
    	}
    }
